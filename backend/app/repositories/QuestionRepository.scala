...
/** Repository for managing Question entities in the MongoDB database.
  *
  * @param config
  *   Application configuration for database connection.
  * @param ec
...
class QuestionRepository @Inject() (
  ...
  /** Retrieves all questions from the database.
    *
    * @return
    *   A Future sequence of Question objects.
    */
  ...
  def getQuestion(question_id: String): Future[Option[Question]] = {
    collection
      .find(equal("_id", new ObjectId(question_id)))
      .first()
      .toFuture()
      .map { doc =>
        parseDocumentToQuestion(doc)
  ...
  def checkAnswer(question_id: String, answer: String): Future[Boolean] = {
    collection
      ...
      .map { doc =>
        ...
        val correctAnswer = (questionJson \ "options")
          .as[List[QuestionOption]]
          .find(_.isCorrect.getOrElse(false))
          .map(_.text)
  ...
  /** Inserts a new question into the database and retrieves it with its
    * generated MongoDB ObjectId.
    *
    * @param question
    *   The `QuestionDto` object containing the question details to be inserted.
    * @return
    *   A `Future` containing the inserted `Question` with its MongoDB ObjectId.
    * @throws Exception
    *   If the operation fails at any stage.
    */
  ...
  private def parseDocumentToQuestion(doc: Document): Option[Question] = {
    try {
      ...
      val question = Question(
        id = id.toString(),
        content = (questionJson \ "content").as[String],
        options = (questionJson \ "options").as[List[QuestionOption]]
      )
      Some(question)
    } catch {
      case ex: Throwable =>
