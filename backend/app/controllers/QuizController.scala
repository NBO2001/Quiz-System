...
class QuizController @Inject()(val controllerComponents: ControllerComponents, questionRepository: QuestionRepository)(implicit val ec: ExecutionContext)
  ...
  def getById(question_id: String, answered: Boolean = true): Action[AnyContent] = Action.async {
    questionRepository.getQuestion(question_id).map {
      case Some(q) =>
        if (answered) {
          Ok(Json.toJson(q.copy(options = q.options.map(_.copy(isCorrect = None)))))
        } else {
          Ok(Json.toJson(q))
        }
      case None => NotFound(Json.obj("error" -> s"Question with ID $question_id not found"))
    }
  }

  def checkAnswer(question_id: String): Action[JsValue] = Action.async(parse.json) { request =>
    request.body.validate[OptionDto] match {
      case JsSuccess(answer, _) =>
        questionRepository.checkAnswer(question_id, answer.text).map { isCorrect =>
