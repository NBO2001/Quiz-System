...
def test_should_find_question_number(question_input_test: tuple[str, int]):
    ...
    question = QuestionType.from_raw(discipline="", options=[], raw_statement=stmt)
...
def test_should_find_statement(question_input_test: tuple[str, int]):
    stmt_raw, stmt_correct = question_input_test

    question = QuestionType.from_raw(discipline="", options=[], raw_statement=stmt_raw)
